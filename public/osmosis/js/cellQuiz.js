import { CELL_QUIZ, QUIZ_SECTIONS } from "./cellQuizData.js";
import { sectionLabel, renderSessionSummary } from "./membraneQuizSummary.js";
import { downloadWord, printSheet } from "./cellQuizExport.js";
import {
  DIFFICULTY_LEVELS,
  difficultyLevel,
  seededShuffle,
...
const UI = {
  en: {
    ...
    revBandFair: "Mixed performance: re-read cell notes for weaker topics, then regenerate.",
    revBandLow:
      "Several concepts need consolidation. Review cell theory, organelles, cell types, and biomolecules before the next round.",
  ...
  zh: {
    ...
    revBandLow: "多个概念仍需巩固。请先温习细胞学说、细胞器、细胞类型与生物分子。",
  ...
  "zh-Hant": {
    ...
    revBandLow: "多個概念仍需鞏固。請先溫習細胞學說、細胞器、細胞類型與生物分子。",
...
export function initCellQuiz() {
  ...
  function updateBankSummary() {
    ...
    const pool = filterQuizPool(CELL_QUIZ, { sections, formats, difficulty });
  ...
  function generate() {
    ...
    const pool = filterQuizPool(CELL_QUIZ, {
      sections,
      formats,
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => initCellQuiz());
} else {
  initCellQuiz();
}
