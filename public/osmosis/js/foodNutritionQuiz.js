import { FOOD_NUTRITION_QUIZ, QUIZ_SECTIONS } from "./foodNutritionQuizData.js";
import { sectionLabel, renderSessionSummary } from "./membraneQuizSummary.js";
import { downloadWord, printSheet } from "./foodNutritionQuizExport.js";
import {
  DIFFICULTY_LEVELS,
  difficultyLevel,
  seededShuffle,
...
const UI = {
  en: {
    ...
    txtKeyboardHint:
      "You may press A, B, C, D on your keyboard to choose your answers for MCQ, and T, F for true and false questions. Hover over a question and press Enter (Windows) or Return (macOS) to check that question only.",
    ...
    revBandFair: "Mixed performance: re-read food & nutrition notes for weaker topics, then regenerate.",
    revBandLow:
      "Several concepts need consolidation. Review biomolecule basics, hydrolysis/condensation, denaturation, and food sources before the next round.",
  ...
  zh: {
    ...
    txtKeyboardHint: "选择题可按键盘 A、B、C、D 作答；判断题可按 T、F 作答。鼠标悬停在某题上时，按 Enter（Windows）或 Return（macOS）仅检查该题。",
    btnSummary: "学习摘要",
    quizCheck: "检查答案",
    empty: "请先按「生成题目」。",
    alertNoQuiz: "请先生成题目。",
    ...
    revBandLow: "多个概念仍需巩固。请先温习生物分子基础、水解/缩合、变性与食物来源。",
  ...
  "zh-Hant": {
    ...
    txtKeyboardHint: "選擇題可按鍵盤 A、B、C、D 作答；判斷題可按 T、F 作答。滑鼠懸停在某題上時，按 Enter（Windows）或 Return（macOS）僅檢查該題。",
    btnSummary: "學習摘要",
    quizCheck: "檢查答案",
    empty: "請先按「產生題目」。",
    alertNoQuiz: "請先產生題目。",
    ...
    revBandLow: "多個概念仍需鞏固。請先溫習生物分子基礎、水解/縮合、變性與食物來源。",
...
export function initFoodNutritionQuiz() {
  ...
  function updateBankSummary() {
    ...
    const pool = filterQuizPool(FOOD_NUTRITION_QUIZ, { sections, formats, difficulty });
  ...
  function generate() {
    ...
    const pool = filterQuizPool(FOOD_NUTRITION_QUIZ, {
      sections,
      formats,
...
document.addEventListener("DOMContentLoaded", () => initFoodNutritionQuiz());
