import { ENZYME_QUIZ, QUIZ_SECTIONS } from "./enzymeQuizData.js";
import { sectionLabel, renderSessionSummary } from "./membraneQuizSummary.js";
import { downloadWord, printSheet } from "./enzymeQuizExport.js";
import {
  DIFFICULTY_LEVELS,
  difficultyLevel,
  seededShuffle,
...
const UI = {
  en: {
    ...
    revBandFair: "Mixed performance: re-read enzyme notes for weaker topics, then regenerate.",
    revBandLow:
      "Several concepts need consolidation. Review enzyme basics, mechanism, factors, and denaturation before the next round.",
  ...
  zh: {
    ...
    revBandLow: "多个概念仍需巩固。请先温习酶的基本概念、作用机制、影响因素与变性。",
  ...
  "zh-Hant": {
    ...
    revBandLow: "多個概念仍需鞏固。請先溫習酶的基本概念、作用機制、影響因素與變性。",
...
export function initEnzymeQuiz() {
  ...
  function updateBankSummary() {
    ...
    const pool = filterQuizPool(ENZYME_QUIZ, { sections, formats, difficulty });
  ...
  function generate() {
    ...
    const pool = filterQuizPool(ENZYME_QUIZ, {
      sections,
      formats,
...
document.addEventListener("DOMContentLoaded", () => initEnzymeQuiz());
