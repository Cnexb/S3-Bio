...
function buildDocBody(questions, answersMode) {
  ...
  questions.forEach((q, i) => {
    ...
    body += stemToHtml(q.stem, q.items, q.stemTables?.length ? q.stemTables : q.stemTable);
    if (q.optionGrid) body += optionGridHtml(q.optionGrid);
    ...
    if (!answersMode) {
      if (fmt === "fill" && getFillLines(q).length) {
        ...
        body += "</ol>";
      } else if (q.options?.length && !q.optionGrid) {
        body += "<ul>";
...
export function downloadWord(questions, answersMode, lang) {
  ...
  const titleEn = answersMode
    ? "S3 Biology MCQ tests — Answers"
    : "S3 Biology MCQ tests — Questions";
  const titleZh = answersMode ? "S3 生物選擇題 — 答案" : "S3 生物選擇題 — 試題";
  ...
  a.download = (answersMode ? "s3_bio_answers_" : "s3_bio_questions_") + ts + ".doc";
  a.click();
