import { QUIZ_ITEMS, QUIZ_SECTIONS, QUIZ_CHAPTERS } from "./quizData.js";
...
const UI = {
  en: {
    ...
    hSettings: "Choose tests",
    lblTypes: "Tests (multi-select)",
    lblFormats: "Question types",
    bankSummaryTitle: "Selected tests",
    bankAvailable: "Questions that will be included",
    bankByTopic: "By test",
    bankByType: "By question type",
    bankMatrix: "Topic × type",
    bankNone: "Select at least one test. Every question in that file is included.",
    lblCount: "Number of questions (1–50)",
    lblDiff: "Difficulty",
    lblSeed: "Random seed (optional)",
    btnGenerate: "Start test",
    ...
    empty: "Select one or more tests, then start.",
    alertNoQuiz: "Start a test first.",
    alertNoTypes: "Select at least one test.",
    alertNoFormats: "Select at least one question type.",
    alertNoMatch: "No questions in the selected tests.",
    alertPoolLimited:
    ...
    revBandFair: "Mixed performance: re-read notes for weaker tests, then try those files again.",
    revBandLow:
      "Several concepts need consolidation. Review the selected tests in your notes before the next round.",
  ...
  zh: {
    appSubtitle: "每个 Word 档案 = 一份测验 · 可多选",
    hSettings: "选择测验",
    lblTypes: "测验（可多选）",
    lblFormats: "题型",
    bankSummaryTitle: "已选测验",
    bankAvailable: "将会包含的题数",
    bankByTopic: "按测验",
    bankByType: "按题型",
    bankMatrix: "课题 × 题型",
    bankNone: "请至少选择一份测验。该档案内全部题目都会纳入。",
    lblCount: "题数（1–50）",
    lblDiff: "难度",
    lblSeed: "随机种子（可留空）",
    btnGenerate: "开始测验",
    ...
    btnSummary: "学习摘要",
    quizCheck: "检查答案",
    empty: "请先选择一份或多份测验，然后开始。",
    alertNoQuiz: "请先开始测验。",
    alertNoTypes: "请至少选择一份测验。",
  ...
  "zh-Hant": {
    appSubtitle: "每個 Word 檔 = 一份測驗 · 可多選",
    hSettings: "選擇測驗",
    lblTypes: "測驗（可多選）",
    lblFormats: "題型",
    bankSummaryTitle: "已選測驗",
    bankAvailable: "將會包含的題數",
    bankByTopic: "按測驗",
    bankByType: "按題型",
    bankMatrix: "課題 × 題型",
    bankNone: "請至少選擇一份測驗。該檔案內全部題目都會納入。",
    lblCount: "題數（1–50）",
    lblDiff: "難度",
    lblSeed: "隨機種子（可留空）",
    btnGenerate: "開始測驗",
    ...
    btnSummary: "學習摘要",
    quizCheck: "檢查答案",
    empty: "請先選擇一份或多份測驗，然後開始。",
    alertNoQuiz: "請先開始測驗。",
    alertNoTypes: "請至少選擇一份測驗。",
...
export function initQuiz() {
  ...
  function updateBankSummary() {
    ...
    const pool = filterQuizPool(QUIZ_ITEMS, { sections, formats: ["mcq"], difficulty: "all" });
    ...
    els.bankSummary.innerHTML = `
      ...
      <ul class="space-y-1 text-on-surface max-h-48 overflow-y-auto">${topicRows}</ul>`;
  }
  ...
  function bindFilterListeners() {
    ...
    panel.addEventListener("change", (e) => {
      const t = e.target;
      if (t?.matches?.("input.chapter-check")) {
        const chapter = t.closest(".chapter-group");
        chapter?.querySelectorAll("input.topic-check").forEach((box) => {
          box.checked = t.checked;
        });
        updateBankSummary();
        return;
      }
      if (t?.matches?.("input.topic-check")) {
        const chapter = t.closest(".chapter-group");
        const boxes = [...(chapter?.querySelectorAll("input.topic-check") || [])];
        const head = chapter?.querySelector("input.chapter-check");
  ...
  function initMeta() {
    if (els.typeChecks) {
      const kept = new Set(selectedSections());
      const chapters = Array.isArray(QUIZ_CHAPTERS) && QUIZ_CHAPTERS.length
        ? QUIZ_CHAPTERS
        : [{ id: "all", label: "Tests", labelZh: "測驗", sections: QUIZ_SECTIONS.map((s) => s.id) }];
      els.typeChecks.innerHTML = chapters
        .map((ch) => {
          const title = isChineseUI(lang) ? ch.labelZh || ch.label : ch.label;
          const tests = (ch.sections || [])
            .map((sid) => QUIZ_SECTIONS.find((s) => s.id === sid))
            .filter(Boolean);
          if (!tests.length) return "";
          const rows = tests
            .map((sec) => {
              const label = isChineseUI(lang) ? sec.labelZh : sec.label;
              const n = countInSection(sec.id);
              return `
              <label class="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-container-low border border-outline-variant/20 cursor-pointer">
                <input type="checkbox" class="topic-check rounded border-outline-variant text-primary focus:ring-primary" value="${escHtml(sec.id)}" />
            ...
            .join("");
          return `
            <div class="chapter-group" data-chapter="${escHtml(ch.id)}">
              <label class="chapter-head font-label-bold text-on-surface cursor-pointer">
                <input type="checkbox" class="chapter-check rounded border-outline-variant text-primary focus:ring-primary" />
        ...
        .join("");
      kept.forEach((id) => {
        const box = els.typeChecks.querySelector(`input.topic-check[value="${id}"]`);
        if (box) box.checked = true;
      });
      els.typeChecks.querySelectorAll(".chapter-group").forEach((group) => {
        const boxes = [...group.querySelectorAll("input.topic-check")];
        const head = group.querySelector("input.chapter-check");
    ...
    bindFilterListeners();
    const panel = document.getElementById("settings-panel");
    if (panel && panel.dataset.bulkListeners !== "1") {
      panel.dataset.bulkListeners = "1";
      document.getElementById("btn-select-all")?.addEventListener("click", () => {
        els.typeChecks?.querySelectorAll("input.topic-check, input.chapter-check").forEach((box) => {
          box.checked = true;
  ...
  function selectedSections() {
    return Array.from(els.typeChecks?.querySelectorAll("input.topic-check:checked") || []).map((x) => x.value);
  }
  ...
  function renderQuiz() {
    ...
    lastQuestions.forEach((q, idx) => {
      ...
      figures.forEach((im) => {
        ...
        fig.innerHTML = `<img src="${escHtml(im.src)}" alt="${escHtml(im.alt || "")}" loading="lazy" />
          ${im.caption ? `<figcaption class="text-body-sm text-on-surface-variant mt-2">${escHtml(im.caption)}</figcaption>` : ""}`;
        wrap.appendChild(fig);
      });

      const stem = document.createElement("div");
      stem.className = "mb-4";
      stem.innerHTML = stemToHtml(q.stem, q.items, q.stemTables?.length ? q.stemTables : q.stemTable);
      wrap.appendChild(stem);
      if (q.optionGrid) {
        const grid = document.createElement("div");
        grid.innerHTML = optionGridHtml(q.optionGrid);
      ...
      if (fmt === "fill" && countFillBlanks(q) > 0) {
        ...
        q.options.forEach((opt) => {
          ...
          if (opt.image || opt.wingding || !q.optionGrid) {
            const text = document.createElement("span");
            text.className = "font-body-md text-on-surface flex-1 text-left";
            if (opt.image) {
              text.innerHTML = `<img class="quiz-opt-img" src="${escHtml(opt.image)}" alt="${escHtml(opt.key)}" />`;
            } else if (opt.wingding) {
              const n = parseInt(String(opt.wingding), 16);
              const ascii = n >= 0xf000 ? n - 0xf000 : n;
              let label = "";
              if (ascii >= 0x8c && ascii <= 0x95) label = String(ascii - 0x8b);
              else if (ascii >= 0x81 && ascii <= 0x8a) label = String(ascii - 0x80);
              else label = String(ascii);
              text.innerHTML = `<span class="quiz-callout">${escHtml(label)}</span>`;
            } else {
